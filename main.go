...
import (
	...
	"github.com/VladHrytsaiuk/wegas-finance/backend/appsetup"
...
func startApp() {
	...
	if err := appsetup.RunMigrations(db); err != nil {
		log.Fatal("❌ Migration error:", err)
	}

	appsetup.StartSystemSeed(db)

	runtime, err := appsetup.BuildRuntime(cfg, db)
	if err != nil {
	...
	r := appsetup.NewRouter(cfg, runtime.Controllers)
