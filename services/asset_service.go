...
func NewAssetService(repo repositories.AssetRepository, txRepo repositories.TransactionRepository) *AssetService {
  return &AssetService{repo: repo, txRepo: txRepo}
}
...
func (s *AssetService) UploadPhoto(assetID string, file multipart.File, header *multipart.FileHeader, user *models.User) (string, error) {
  ...
  if err != nil { return "", errors.New("asset not found") }

  uploadDir := "./uploads/assets"
  if err := os.MkdirAll(uploadDir, os.ModePerm); err != nil { return "", err }

  img, _, err := image.Decode(file)
  if err != nil { return "", errors.New("invalid image format") }
  ...
  if err := jpeg.Encode(out, img, &jpeg.Options{Quality: 80}); err != nil { return "", err }
  ...
  if err := s.repo.AddPhotoToGallery(galleryPhoto); err != nil { return "", err }

  if asset.Photo == "" {
    _ = s.repo.UpdatePhoto(assetID, publicPath)
  }
...
func (s *AssetService) RemovePhoto(assetID string, path string, user *models.User) error {
  ...
  if err := s.repo.RemovePhoto(assetID, path); err != nil { return err }
...
func (s *AssetService) UploadDocument(assetID string, file multipart.File, header *multipart.FileHeader, user *models.User) (*models.AssetDocument, error) {
  // 1. Перевіряємо права
  _, err := s.repo.GetByID(assetID, user.FamilyID)
  if err != nil { return nil, errors.New("asset not found") }
  ...
  if err := os.MkdirAll(uploadDir, os.ModePerm); err != nil { return nil, err }

  // 3. Зберігаємо оригінальну назву і розширення
  originalExt := strings.ToLower(filepath.Ext(header.Filename))
  if originalExt != ".pdf" && originalExt != ".doc" && originalExt != ".docx" {
    // Якщо хочеш дозволити ТІЛЬКИ PDF, розкоментуй це:
  ...
  // Копіюємо файл як є (без стиснення)
  if _, err := io.Copy(out, file); err != nil { return nil, err }
  ...
  if err := s.repo.AddDocument(doc); err != nil {
    os.Remove(filePath) // Відкат, якщо помилка БД
    return nil, err
  }
...
func (s *AssetService) RemoveDocument(assetID string, documentID string, user *models.User) error {
  ...
  if err := s.repo.RemoveDocument(assetID, documentID); err != nil { return err }

  // Фізично видаляємо файл
  cleanPath := strings.TrimPrefix(doc.Path, "/")
  _ = os.Remove(cleanPath)
...
func (s *AssetService) CalculateDepreciation(a *models.Asset) int64 {
  if a.IsSold { return a.SoldPrice }
  if a.CurrentPrice > 0 { return a.CurrentPrice }

  now := time.Now().UnixMilli()
  ageInYears := float64(now-a.PurchaseDate) / (1000 * 60 * 60 * 24 * 365.25)
  
  life := 5.0
  if a.EstimatedLife > 0 { life = float64(a.EstimatedLife) / 12.0 }
  if ageInYears > life { return 0 }

  val := float64(a.Price)
  if a.InitialValue > 0 { val = float64(a.InitialValue) }
