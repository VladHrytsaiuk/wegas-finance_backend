...
| **utils** | 64.4% | 🟡 Добре (основні парсери та криптографія) |
| **repositories** | 48.7% | 🟡 Середнє (основні CRUD операції) |
| **pkg/telegram** | 46.7% | 🟡 Базове тестування клієнта |
| **controllers** | 40.4% | 🟡 Середнє (всі основні API endpoint-и) |
| **services** | 35.7% | 🔴 Потребує уваги (багато складної логіки) |
| **services/parsers** | 29.8% | 🔴 Потребує більше прикладів виписок |
...
- [ ] **StorageService:** Тестування логіки завантаження та видалення файлів (mock S3/local storage).
- [ ] **TransactionRepository:** Додати складні тести для `BatchCreate` з великою кількістю зв'язків.

### Етап 11: Real-time та Інфраструктура
- [ ] **WSController:** Тестування WebSocket з'єднань через mock-клієнтів.
- [ ] **WSHub:** Тестування розсилки (broadcast) повідомлень по родинах.
- [ ] **Routes:** Тестування цілісності таблиці маршрутизації (перевірка наявності всіх зареєстрованих шляхів).

### Етап 12: Стрес-тести та Edge Cases
- [ ] Тестування поведінки при розриві з'єднання з БД.
- [ ] Перевірка лімітів завантаження великих PDF/CSV файлів.
- [ ] Перевірка безпеки (Mass Assignment, IDOR) на рівні всіх контролерів.
