...
func accountBalanceDelta(txType string, amount int64) int64 {
	switch txType {
...
func (r *txRepo) Create(t *models.Transaction, items []models.TransactionItem, tagIDs []string, newAsset *models.Asset) error {
	return r.db.Transaction(func(txDB *gorm.DB) error {
		...
		if t.AccountID != "" && !t.IsForgiveness {
			change := accountBalanceDelta(t.Type, t.Amount)
...
func (r *txRepo) BatchCreate(transactions []models.Transaction) (int, error) {
	...
	err := r.db.Transaction(func(txDB *gorm.DB) error {
		...
		for index := range transactions {
			...
			account, found := accounts[t.AccountID]
			if !found {
				if err := txDB.Set("gorm:query_option", "FOR UPDATE").First(&account, "id = ?", t.AccountID).Error; err != nil {
					return fmt.Errorf("account not found: %w", err)
				}
				if account.FamilyID != t.FamilyID {
					return fmt.Errorf("account does not belong to transaction family")
				}
				accounts[t.AccountID] = account
			}
			if account.IsSynced && t.ExternalID == "" {
				return fmt.Errorf("🚫 Цей рахунок (%s) синхронізується автоматично. Ручне додавання заборонено.", account.Name)
			}
			...
			if !t.IsForgiveness {
				balanceChanges[t.AccountID] += accountBalanceDelta(t.Type, t.Amount)
			}
			if err := r.applyCounterpartyBalance(txDB, t.CounterpartyID, t.Currency, calculateDebtDelta(t.Type, t.Amount)); err != nil {
				return err
			}
		}
		...
		for accountID, change := range balanceChanges {
			...
			if err := txDB.Model(&models.Account{}).Where("id = ?", accountID).
				Update("balance", gorm.Expr("balance + ?", change)).Error; err != nil {
				return err
			}
		}
